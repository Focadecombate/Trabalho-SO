...
# IMPORTA O WEBBROWSER PARA ABRIR A PAGINA DO GRÁFICO COM O NAVEGADOR PADRÃO
import webbrowser
...
def fifo():
    ...
    relogio = 0
    for x in range(0, n):
        # INCREMENTA O RELOGIO COM O TEMPO DE EXECUÇÃO DO PROCESSO
        relogio += tempos[x]
        # INCREMENTA A SOMA COM O TEMPO FINAL DO PROCESSO (RELOGIO) - O TEMPO DE ENTRADA
...
def rr():
    # COPIA A LISTA DE ENTRADAS PARA UMA NOVA LISTA, QUE SERÁ ORDENADA
    entradas = list(tmpEnt)
    tempos = list(tmpExe)  # MESMA IDEIA DE CIMA
    relogio = 0
    # CRIAMOS UMA LISTA ONDE A CADA EXECUÇÃO IREMOS INCREMENTAR O TEMPO QUE FOI EXECUTADO
    processados = [0]*n
    # CRIAMOS UMA LISTA DE 0/1 PARA SABER QUAIS PROCESSOS JA ENTRAM
    entraram = [0]*n
    ...
    def entra():
        # ADICIONA OS TEMPOS QUE NÃO ENTRARAM E MENORES OU IGUAL AO RELOGIO NA FILA
        for x in range(0, n):
            if entradas[x] <= relogio and entraram[x] == 0:
                # print "Entrou ", x
                # OS PROCESSOS QUE JÁ ENTRARAM, RECEBEM 1, ASSIM SÓ ENTRAM NOVAMENTE NA FILA EM CASO DE PREEPÇÃO
                entraram[x] = 1
    ...
    for processo in fila:
        # print "=====", processo, "======"
        # VARIÁVEL FALTA RECEBE O TEMPO DO PROCESSO - O QUE JÁ FOI PROCESSADO
        falta = tempos[processo]-processados[processo]
        if falta > quantum:  # SE FALTA MAIS QUE O QUANTUM ENTRA NO BLOCO
            # RELOGIO INCREMENTA O QUANTUM, POIS IRÁ EXECUTAR TODO O TEMPO DO QUANTUM
            relogio += quantum
            entra()  # VERIFICA SE ALGUM PROCESSO CHEGA DURANTE A EXECUÇÃO ATUAL
            # INCREMENTA EM UM QUANTUM O QUE JÁ FOI PROCESSADO DO PROCESSO ATUAL
            ...
            # COMO O PROCESSO NÃO FOI EXECUTADO TOTALMENTE, ELE VOLTA PARA O FIM DA FILA DE EXECUÇÃO
            fila.append(processo)
            relogio += 1  # ADIOCIONA AO RELOGIO O TEMPO DA SOBRECARGA
        # NESSE CASO VERIFICAMOS SE FALTA ALGUM TEMPO ENTRE 0 E O QUANTUM A SER EXECUTADO
        elif falta <= quantum and falta > 0:
            relogio += falta  # INCREMENTA O RELÓGIO O TEMPO QUE FALTA
            entra()  # VERIFICA SE ALGUM PROCESSO CHEGA DURANTE A EXECUÇÃO ATUAL
            # INCREMENTA O QUE FALTA AO QUE JÁ FOI PROCESSADO DO PROCESSO ATUAL
            processados[processo] += falta
            # INCREMENTA A SOMA COM O TURNAROUND DO PROCESSO
...
def edf():
    ...
    def entra():
        for x in range(0, n):  # ADICIONA OS TEMPOS QUE NÃO ENTRARAM E MENORES OU IGUAL AO RELOGIO
            if entradas[x] <= relogio and entraram[x] == 0:
                # print "Entrou ", x
...
while cmd != 0:
    if cmd == 1:
        ...
        saida = {'labels': ["FIFO", "SJF", "Round Robin", "EDF"],
                 'datasets': [{'data': [fifo(), sjf(), rr(), edf()]}]}
